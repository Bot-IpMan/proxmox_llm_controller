...
Коли просиш агента виконати дію в BlissOS, чітко вкажи три речі:

1. **Ціль** – що саме має статись ("відкрий налаштування Wi-Fi", "натисни кнопку 'OK'", "запусти застосунок Chrome").
2. **Контекст** – де знаходиться потрібний елемент ("на головному екрані", "у верхній шторці", "в меню налаштувань"). Якщо знаєш координати або команду `adb shell`, додай їх.
3. **Очікуваний результат** – як зрозуміти, що дія виконана ("має з'явитися список мереж", "переконайся, що перемикач 'Wi-Fi' увімкнено").

Приклади природних підказок:

- «Підключись до BlissOS по ADB і відкрий меню налаштувань Wi-Fi. Якщо потрібно, використай `adb shell input keyevent` для навігації.»
- «На підключеному екрані натисни тапом координати 960x540, щоб активувати центральну кнопку, а потім перевір, чи з'явилось діалогове вікно підтвердження.»
- «Виконай команду `adb shell am start -n org.mozilla.firefox/.App` і повідом, коли браузер запуститься.»

Формулюй запити звичайною мовою, але додавай технічні деталі, коли це критично для точності. Якщо потрібна послідовність дій, попроси агента побудувати план («спочатку активуй ADB, потім відкрий шторку, потім натисни перемикач Wi-Fi»).
