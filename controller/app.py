    ...
    try:
        with spec_path.open("r", encoding="utf-8") as handle:
            try:
                ...
                log.exception(
                    "Invalid JSON in BlissOS OpenAPI specification at %s", spec_path
                )
                raise ValueError(
        ...
        log.error(
            "Permission denied when reading BlissOS OpenAPI specification at %s", spec_path
        )
        raise PermissionError(
            "Permission denied while reading the BlissOS OpenAPI specification. "
            "Adjust file permissions or mount a readable file."
        ) from exc
...
def bliss_openapi_spec() -> Dict[str, Any]:
    ...
    try:
        ...
        path = BLISS_OPENAPI_PATH or BLISS_OPENAPI_ENV or "/app/openapi_bliss.json"
        detail = (
            "BlissOS OpenAPI specification exists but cannot be read due to file permissions. "
            f"Adjust permissions or mount a readable file at '{path}'."
        )
...
class BlissOpenAPIStatus(BaseModel):
    ...
    readable: bool = Field(
        description="True if the controller has read permission for the file.",
    )
