...
### Контейнер `ollama-init` «чекає» в статусі *Waiting*

Сервіс `ollama-init` запускає скрипт [`scripts/ollama-init.sh`](scripts/ollama-init.sh), який автоматично
завантажує моделі для Ollama. Список моделей передається через змінну `OLLAMA_AUTO_PULL_MODELS`; якщо вона порожня,
скрипт використовує значення за замовчуванням `qwen2.5-coder:7b nomic-embed-text`. Обидві моделі досить великі (десятки
гігабайт), тому контейнер може довго перебувати у стані *Waiting*, поки Ollama не завершить завантаження.

Що можна зробити:

1. **Перевірити прогрес завантаження.** Команда `docker compose logs -f ollama-init` показує, яку модель завантажують
   зараз і чи вже закінчено процес. Якщо ви запускаєте стек із нестандартним `COMPOSE_PROJECT_NAME`, замініть назву
   сервісу на відповідну (`docker compose ps`, щоб переконатися).
2. **Скоротити або очистити список.** Додайте у `.env` власний перелік моделей, наприклад
   `OLLAMA_AUTO_PULL_MODELS="phi3:mini-instruct"`, або задайте порожній рядок (`OLLAMA_AUTO_PULL_MODELS=""`), якщо
   хочете виконати `ollama pull` вручну вже після старту контейнера.
3. **Задати конкретні моделі.** Якщо вам потрібні інші моделі за замовчуванням, пропишіть їх через пробіл, наприклад
   `OLLAMA_AUTO_PULL_MODELS="llama3:8b-instruct nomic-embed-text"`.

Контейнер `ollama-init` завершує роботу відразу після успішного завантаження всіх перелічених моделей, тому тривалість
його виконання напряму залежить від вибраного списку.
...
Той самий override передає GPU також сервісу `proxmox-controller`, щоб локальний генератор
Hugging Face (`BLISS_LLM_PROVIDER=huggingface`) міг використовувати CUDA всередині контейнера.
