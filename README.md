...
3. Додайте приватний SSH‑ключ, який має доступ до вашого вузла Proxmox:
   ...
   * для ручного підключення всередині контейнера використовуйте `ssh -i /keys/pve_id_rsa root@IP` або налаштуйте `~/.ssh/config`/симлінк, щоб не передавати `-i` щоразу;
   * `proxmox-controller` читає цей файл автоматично через змінну середовища `PVE_SSH_KEY_PATH`.

4. Запустіть стек командою:
